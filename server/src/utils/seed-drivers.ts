...
export const driversPayload: Driver[] = [
  {
    id: 1,
    name: "Homer Simpson",
...
export const seedDrivers = async () => {
  const driversColletion = await MongoHelper.getCollection("drivers");
  await driversColletion.deleteMany({});
  await driversColletion.insertMany(driversPayload);
}
